...
# ------------------------- I/O & helpers ------------------------
@st.cache_data
def load_surface_csv(p: Path, source_tag: str | None = None) -> pd.DataFrame:
    if not p.exists():
        return pd.DataFrame()
    df = pd.read_csv(p)
    ...
    if "DLAT" in df.columns:
        rename_map["DLAT"] = "LATITUDE"
    if "DLONG" in df.columns:
        rename_map["DLONG"] = "LONGITUDE"
    df = df.rename(columns=rename_map)

    # Ensure numeric columns
    for c in ["LONGITUDE", "LATITUDE", "DEPTH", "FROMDEPTH", "TODEPTH",
              "Cu_ppm", "CU_ORIG", "CU_DL"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")

    # Add SOURCE if missing (useful for categorical coloring)
    if "SOURCE" not in df.columns and source_tag is not None:
        df["SOURCE"] = "orig_only" if source_tag == "ORIG" else "dl_only"
...
surf_orig = load_surface_csv(PATH_SURF_ORIG, "ORIG")
surf_dl   = load_surface_csv(PATH_SURF_DL,   "DL")
...
# ------------------------------ Load data -----------------------------
surf_orig = load_surface_csv(PATH_SURF_ORIG)
surf_dl   = load_surface_csv(PATH_SURF_DL)
...
# longitude / latitude sliders (default to +2° window when possible)
if {"LONGITUDE","LATITUDE"}.issubset(u.columns):
    ...
    lon_default = (lon_min, lon_min + 2)
    lat_default = (lat_min, lat_min + 2)

xr = st.sidebar.slider("Longitude", lon_min, lon_max, lon_default)
yr = st.sidebar.slider("Latitude",  lat_min, lat_max, lat_default)

# View mode choice (source vs value) — matches 09’s numeric/category split
...
# Numeric display options (Portland fixed)
if view_kind == "By value":
    palette = "Portland"
    ...
    max_abs_clip = 500.0; pctl = 95.0; min_abs_val = 0.0
...
NUM_CHOICES = [c for c in ["Cu_ppm","CU_DL","CU_ORIG","DIFF","DIFF_PCT"] if c in base.columns]
default_value_col = "Cu_ppm" if "Cu_ppm" in NUM_CHOICES else (NUM_CHOICES[0] if NUM_CHOICES else None)

if view_kind == "By value":
    if not NUM_CHOICES:
        st.warning("No numeric value columns found (expected one of Cu_ppm, CU_DL, CU_ORIG, DIFF, DIFF_PCT).")
        st.stop()
    value_col = st.sidebar.selectbox("Value to display", NUM_CHOICES,
                                     index=NUM_CHOICES.index(default_value_col))
    # additional numeric filter
    base[value_col] = pd.to_numeric(base[value_col], errors="coerce")
    if min_abs_val > 0 and value_col in ("DIFF","DIFF_PCT"):
...
with tab_points:
    ...
    if view_kind == "By source":
        ...
        if use_basemap:
            ...
            fig.update_traces(marker=dict(size=point_size))
            fig.update_layout(coloraxis_colorbar=dict(title=value_col, ticksuffix=ticksuf))
            fix_extent_mapbox(fig, xr, yr)
        else:
            fig = px.scatter(
                view, x="LONGITUDE", y="LATITUDE",
            ...
            fig.update_layout(
                xaxis_title="Longitude", yaxis_title="Latitude",
                coloraxis_colorbar=dict(title=value_col, ticksuffix=ticksuf),
                height=800, margin=dict(l=0, r=0, t=50, b=10),
            )
            fix_extent_axes(fig, xr, yr)
        st.plotly_chart(fig, use_container_width=True)
                ...
                if use_basemap:
                    ...
                    if map_mode == "Heat (density)":
                        ...
                        fig.update_layout(coloraxis=dict(cmin=vmin, cmax=vmax,
                                                         colorbar=dict(title=value_col, ticksuffix=ticksuf)))
                    else:
                        fig = px.scatter_mapbox(
...
def keep_cols(df: pd.DataFrame, tag: str) -> pd.DataFrame:
    ...
    keep = [c for c in [
        ...
        "Cu_ppm","CU_ORIG","CU_DL","DIFF","DIFF_PCT"
...
if tbl.empty:
    ...
    st.download_button(
        ...
        file_name="surface_record_filtered.csv",
        mime="text/csv",
        type="primary"
    )