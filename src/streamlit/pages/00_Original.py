...
# 放在 00_Original.py 的最顶部、任何 import 之前
import sys
...
# 其余你已有的导入（例如 pyvista_ops、plotly、numpy …）
# import pyvista_ops as pvo
# ...

# ---- Load data (CSV/Parquet → fallback build) ----
cfg = load_cfg()
p_csv = cfg.original_points_all
p_parq = p_csv.with_suffix(".parquet") if p_csv else None

if p_parq and p_parq.exists():
    points_all = pd.read_parquet(p_parq)
elif p_csv and p_csv.exists():
    points_all = pd.read_csv(p_csv)
else:
    # 如果没有现成汇总表：用四张 cleaned 表即时合并（只做展示，不落盘）
    points_all = build_points_views(cfg)["points_all"]

# 保险：统一 dtype
num_cols = ["LONGITUDE", "LATITUDE", "DEPTH", "VALUE"]
for c in num_cols:
    if c in points_all.columns:
        points_all[c] = pd.to_numeric(points_all[c], errors="coerce")
import streamlit as st
import pandas as pd
import numpy as np
from pathlib import Path
...
def load_points_all(path: Path) -> pd.DataFrame:
    """Tall CSV with LONGITUDE, LATITUDE, DEPTH (meters), VALUE, SOURCE."""
    df = pd.read_csv(path)
...
max_points = st.sidebar.number_input("Max points to plot (per dataset)", min_value=1_000, value=50_000, step=5_000)
point_size = st.sidebar.slider("Point size", 1, 8, 3)

# ---------- Voxel params ----------
st.sidebar.subheader("Voxel params")

# === (新增) 按 10m×10m×1m 推导 auto_nx/auto_ny/auto_nz（基于当前视窗） ===
...
min_count = st.sidebar.number_input("Min samples per voxel", min_value=1, value=3, step=1)  # ← 默认 3
...
if view_mode == "Points":
    ...
    # Mask no-data cells; pass text for robust hover
    Z2d = out["Z2d"].copy()
    Z2d[~finite] = np.nan
    fig = go.Figure()
    fig.add_trace(go.Surface(
        x=out["X2d"], y=out["Y2d"], z=Z2d,
        surfacecolor=out["val_disp_2d"], colorscale=cs, cmin=cmin, cmax=cmax,
        text=out["val_raw_2d"],           # VALUE as text for hover
        customdata=out["val_raw_2d"],     # keep as backup
        hovertemplate=("Lon=%{x:.5f}<br>Lat=%{y:.5f}<br>Depth=%{z:.2f}"
                       "<br>VALUE=%{text:.4g}<extra></extra>"),
