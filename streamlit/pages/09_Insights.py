...
# ---------------------------- I/O helpers ----------------------------
@st.cache_data
def read_drill(p: Path) -> pd.DataFrame:
    if not p.exists():
        return pd.DataFrame()
    df = pd.read_csv(p)
...
@st.cache_data
def read_surface(p: Path) -> pd.DataFrame:
    if not p.exists():
        return pd.DataFrame()
    df = pd.read_csv(p)
    df = df.loc[:, ~df.columns.duplicated()]
    # Normalize DLAT/DLONG if present
...
# Load once
dh_overlap  = read_drill(DH_OVERLAP)
dh_dlonly   = read_drill(DH_DLONLY)
dh_all      = read_drill(DH_ALL)
sf_all      = read_surface(SF_ALL)
sf_overlap  = read_surface(SF_OVERLAP)
sf_dlonly   = read_surface(SF_DLONLY)
